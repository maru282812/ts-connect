...
--
-- 【実行前提】
--   1. Supabase プロジェクトが起動していること
--   2. 本ファイル単体でも companies / users / posts / applications を投入可能
--
...
begin
  ...
  -- ==========================================================
  -- STEP 0: companies を投入（seed.sql 未実行環境の保険）
  -- ==========================================================
  ...
  -- ---- 1. 経費精算レシート整理 ----
  -- 状態: OPEN / 締切近い(3日後) / 担当者あり / サムネあり / 募集1名 / requirements あり
  ...
  -- ---- 3. Webサイト軽微修正 ----
  -- 状態: OPEN / 参考URL（本文内） / 募集3名 / サムネあり / requirements あり
  INSERT INTO public.posts (
    id, company_id, created_by_user_id,
    title, body, post_type, post_status,
    price_text, contact_person_name,
  ...
  -- ---- 4. アンケート結果レポート作成 ----
  -- 状態: OPEN / 締切近い(5日後) / 担当者あり / サムネなし / requirements あり
  INSERT INTO public.posts (
    id, company_id, created_by_user_id,
    title, body, post_type, post_status,
    price_text, contact_person_name,
  ...
  -- ---- cas5. Java 例外設計の相談 ----
  -- 状態: OPEN / 参考URL（本文内） / 投稿者: user4(高橋健太)
  INSERT INTO public.posts (
    id, company_id, created_by_user_id,
    title, body, post_type, post_status,
    published_at
  ...
  -- ---- cas8. アンケート設問レビュー ----
  -- 状態: OPEN / 参考URL（本文内） / 投稿者: user3(鈴木一郎)
  INSERT INTO public.posts (
    id, company_id, created_by_user_id,
    title, body, post_type, post_status,
    published_at
  ...
  -- ==========================================================
  -- STEP 5.5: 既存ユーザー別の追加投稿 8件
  -- ==========================================================
  -- 目的:
  --   ・管理者3名 / 一般ユーザー5名それぞれの投稿表示を確認
  --   ・OFFICIAL / CASUAL の両タイプを確認
  --   ・DRAFT / OPEN / IN_PROGRESS / CLOSED の状態差分を確認
  ...
  INSERT INTO public.posts (
    ...
    E'本案件は確認用の終了済みデータです。応募導線ではなく、アーカイブ表示や詳細表示の確認に使います。',
    'OFFICIAL', 'CLOSED',
    '10,000円',
    '管理者C',
  ...
  INSERT INTO public.posts (
    ...
    E'すでに相談相手は見つかっていますが、IN_PROGRESS 表示確認用として残しています。',
    'CASUAL', 'IN_PROGRESS',
    now() - interval '2 days'
  ) ON CONFLICT (id) DO NOTHING;
  ...
  RAISE NOTICE '    OPEN: 5件 / DRAFT: 2件 / IN_PROGRESS: 1件 / CLOSED: 3件';
